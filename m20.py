...
while i < len(split) and not error:
    ...
    if v1name in graph_map and v2name in graph_map:
        g.insert_edge(graph_map[v1name], graph_map[v2name], None)
    else:
...
if not error:
    ...
    cycle = False
    for v in g.vertices():
        if v not in worked_on and v not in finished:
            if DFS_cycle(g, v, worked_on, finished):
                cycle = True
                break
    if cycle:
        print("<p>This is a <b>Cyclic</b> graph!</p>")
    else:
        print("<p>This is a <b>Acyclic</b> graph!</p>")
